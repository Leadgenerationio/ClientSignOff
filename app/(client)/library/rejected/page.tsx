...
export default function RejectedLibraryPage() {
  const router = useRouter();
  const { data: session } = useSession();
  const [page, setPage] = useState(1);
  const [selectedCreative, setSelectedCreative] = useState<any>(null);

  // Fetch rejected creatives
  const { data, error, isLoading } = useSWR(
    session?.user?.clientId
      ? `/api/client/${session.user.clientId}/creatives/rejected?page=${page}&limit=${PAGE_SIZE}`
      : null,
    fetcher
  );
  ...
  if (error) return <div>Failed to load rejected creatives</div>;
  ...
  return (
    <div className="container py-8">
      <div className="flex justify-between items-center mb-8">
        ...
        <Button variant="outline" onClick={() => router.push('/library/approved')}>
          View Approved
        </Button>
      </div>
          ...
          >
            ...
            <div className="p-3">
              <p className="text-sm text-gray-500">
                Rejected on {formatDate(creative.approvals[0]?.createdAt)}
              </p>
            </div>
          </div>
        ))}
          ...
          {selectedCreative && (
            <div className="space-y-4">
              ...
              <div className="space-y-4">
                ...
                {/* Feedback */}
                {selectedCreative.approvals[0]?.feedback && (
                  <div className="bg-gray-50 p-4 rounded-lg">
                    <h3 className="text-sm font-medium mb-2">Feedback</h3>
                    <p className="text-sm text-gray-700">{selectedCreative.approvals[0].feedback}</p>
                  </div>
                )}
              </div>
            </div>
